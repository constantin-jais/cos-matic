...
`bolt-cosmatic generate` is deterministic and uses safe-write, so a project can commit its
...
`bolt-cosmatic generate --check` already detects drift (it re-renders in memory and compares
...
- Ship a **reusable CI workflow** (`ci-templates/bolt-cos-matic.yml`) that installs
  the tool and runs `bolt-cosmatic generate --check` and `bolt-cosmatic goals`. Adopters copy it.
- **Dogfood it:** the project's own CI builds `bolt-cosmatic` and runs `bolt-cosmatic generate --check`
  plus `bolt-cosmatic goals` against `examples/minimal`, so the repository fails CI if the
  committed example outputs drift or a hard gate breaks.

## Consequences

- The gate composes with goals: `bolt-cosmatic generate --check` also enforces hard gates
...
- No new runtime surface or dependency: this is the existing `--check` made
  exhaustive plus CI plumbing. A scaffolding command (`bolt-cosmatic init-ci`) remains a
