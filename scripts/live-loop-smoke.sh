...
#
# SAFE BY DEFAULT — runs `bolt-cosmatic loop --dry-run` only (read-only: resolves the repo
...
# PR, then deploys. It therefore requires an explicit opt-in, BOLT_COSMATIC_LIVE=1.
#
...
#   BOLT_COSMATIC_LIVE=1 scripts/live-loop-smoke.sh <issue-number> [title]   # real run (sandbox!)
#
set -euo pipefail
...
export BOLT_COSMATIC_DEPLOY_CANARY="true"
export BOLT_COSMATIC_DEPLOY_PROMOTE="true"
export BOLT_COSMATIC_DEPLOY_ROLLBACK="true"
export BOLT_COSMATIC_DEPLOY_SMOKE="true"
...
if [[ "${BOLT_COSMATIC_LIVE:-0}" != "1" ]]; then
  cat <<EOF
...
To run it against a THROWAWAY issue on a SANDBOX repo:
  BOLT_COSMATIC_LIVE=1 $0 $ISSUE "$TITLE"
EOF
  exit 0
fi

echo "== LIVE run (outward, irreversible — sandbox only) =="
cargo run -q --bin bolt-cosmatic -- loop --issue "$ISSUE" --title "$TITLE"
