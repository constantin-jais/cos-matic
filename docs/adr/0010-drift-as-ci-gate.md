...
`aom generate` is deterministic and uses safe-write, so a project can commit its
...
`aom generate --check` already detects drift (it re-renders in memory and compares
...
- Ship a **reusable CI workflow** (`ci-templates/agent-o-matic.yml`) that installs
  the tool and runs `aom generate --check` and `aom goals`. Adopters copy it.
- **Dogfood it:** the project's own CI builds `aom` and runs `aom generate --check`
  plus `aom goals` against `examples/minimal`, so the repository fails CI if the
  committed example outputs drift or a hard gate breaks.

## Consequences

- The gate composes with goals: `aom generate --check` also enforces hard gates
...
- No new runtime surface or dependency: this is the existing `--check` made
  exhaustive plus CI plumbing. A scaffolding command (`aom init-ci`) remains a
