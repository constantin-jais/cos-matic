...
deploy), each behind its own envelope. A7 chains them into a single `cosmatic loop`
under one global envelope.

## Decision

`cosmatic loop` runs dispatch -> automerge -> deploy and **stops at the first stage
...
- **Global envelope** — on top of each stage's own: a loop kill-switch
  (`cosmatic_LOOP_DISABLED`), a scope-fence, and a global circuit-breaker (max
  iterations).
...
- Each link stays usable standalone (`cosmatic incident|dispatch|automerge|deploy`);
