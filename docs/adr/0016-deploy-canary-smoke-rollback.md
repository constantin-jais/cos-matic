...
`bolt-cosmatic deploy` runs canary -> smoke -> (promote | rollback), governed by the
...
- **Envelope** — kill-switch (`BOLT_COSMATIC_DEPLOY_DISABLED`), scope-fence (a repo
...
- **Seams** — `Deployer` (canary/promote/rollback) and `Smoke` (probe) are
  ...
  (`BOLT_COSMATIC_DEPLOY_{CANARY,PROMOTE,ROLLBACK,SMOKE}`) — sovereign and portable; the
