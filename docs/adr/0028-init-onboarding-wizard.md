# ADR-0028 — Interactive onboarding wizard (`bolt-cosmatic init`)
...
Introduce `bolt-cosmatic init`: an interactive Rust-native wizard that:
...
3. **Prints a manual operator checklist** (L1/L2/L3).
   ...
   - Checklist flags each setup step: commit scaffold, review manifest, run `bolt-cosmatic generate --check`, set repo vars (L2/L3), configure branch protections, etc.
...
- **Safe-write is reinforced:** init is the first place users create artifacts; it must never corrupt existing projects. Same pattern as `bolt-cosmatic generate --force`—safe-write is the default, force is opt-in (and forbidden here, as init always respects existing files).
