...
| **live**    | a **fine-grained PAT** stored as secret `cosmatic_BOT_TOKEN` | a branch pushed by `github.token` does **not** trigger CI, so the gate would never see green checks — the PAT makes the bot's push start the checks | `ANTHROPIC_API_KEY` only when using `fixer=claude` |
...
on the sandbox repo alone** — not your account. `cosmatic_CHECKS_TOKEN` is not a secret
...
2. Flag it as a sandbox so the workflow's guard allows live runs:
   ```bash
   gh variable set cosmatic_SANDBOX --body true --repo <you>/<sandbox>
   ```
3. Create a **fine-grained PAT** (GitHub → Settings → Developer settings →
   ...
   ```bash
   gh secret set cosmatic_BOT_TOKEN --repo <you>/<sandbox>
   ```
...
- **Deploy is a no-op.** The `cosmatic_DEPLOY_*` commands are all `true`; the stage
...
`cosmatic_LOOP_DISABLED`, circuit-breaker, zero-PII audit) holds under a real bot.
