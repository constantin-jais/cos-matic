...
#
# SAFE BY DEFAULT — runs `aom loop --dry-run` only (read-only: resolves the repo
...
# PR, then deploys. It therefore requires an explicit opt-in, AOM_LIVE=1.
#
...
#   AOM_LIVE=1 scripts/live-loop-smoke.sh <issue-number> [title]   # real run (sandbox!)
#
set -euo pipefail
...
export AOM_DEPLOY_CANARY="true"
export AOM_DEPLOY_PROMOTE="true"
export AOM_DEPLOY_ROLLBACK="true"
export AOM_DEPLOY_SMOKE="true"
